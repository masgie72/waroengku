...
submitAdmin = () => {
    ...
    if (firstName == "" || lastName == "" || position == "" || userName == "" || password == "") {
        ...
        db.all(`select * from users where username = '${userName}'`, (err, row) => {
            if (err) throw err
            if (row.length < 1) {
                db.run(`insert into users(first_name, last_name, position, username, password, access_level) 
                    values('${firstName}','${lastName}','${position}','${userName}','${password}', 'admin')`, err => {
                    if (err) throw err
...
submitEditPassword = (id) => {
    ...
    if (prevPassword == "" || newPassword == "") {
        dialog.showMessageBoxSync(
            {
                title: 'ALert',
                type: 'info',
                title: 'Password saat ini dan password baru harus diisi'
            }
        )
    } else {
        db.all(`select * from users where id = '${id}' and password = '${prevPassword}'`, (err, row) => {
            if (err) throw err
            if (row.length < 1) {
                dialog.showErrorBox('Invalid Password', 'Password is valid')
            } else {
        ...
        dialog.showMessageBoxSync(
            {
                title: 'Alert',
                type: 'info',
                message: 'Nama depan, nama belakang, dan posisi harus diisi'
            }
        )
    } else {
        db.run(`update users set first_name ='${firstName}', last_name='${lastName}', position='${position}', 
            phone_number='${phoneNumber}', employee_number='${employeeNumber}', status='${employementStatus}' where id = ${id}`, err => {
            if(err) throw err
            ipcRenderer.send('success:update-user')
        })
    }
