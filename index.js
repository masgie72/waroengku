...
ipcMain.on(
    "success:login",
    (e, msgUserId, msgFirstName, msgPosition, msgAccessLevel) => {
        login = true;
...
modalTableConfig = () => {
    configTableModal = new BrowserWindow({
        ...
        maximizable: false,
        resizable: false,
        icon: './assets/images/icons/logoku.ico'
    });
    ...
    db.all(
        ...
        (err, rows) => {
            if (rows.length < 1) {
                ...
                modalTableConfig()
                db.run(
                    `CREATE TABLE profil(id integer primary key, store_name varchar(150), store_tax_id varchar(150),
                     store_address varchar(150), store_moto varchar(150), store_website varchar(150), phone_number varchar(150),
                      email varchar(100), fax varchar(50), bank_account_one varchar(100),
                       bank_name_one varchar(100), bank_account_two varchar(100), 
                       bank_name_two varchar(100), logo varchar(150))`,(err) => {
                        if (err) throw err
                db.run(`CREATE TABLE users(id integer primary key autoincrement, username varchar(100) not null,
                     password varchar(100) not null, access_level varchar(100) not null, first_name varchar(100) not null,
                      last_name varchar(100) not null, position varchar(100), phone_number varchar(12), 
                      employee_number varchar(50), status varchar(20))`, (err) => {
                        if(err) throw err
                db.run(`CREATE TABLE buyers (id integer primary key autoincrement, name varchar(150), address text,
                     website varchar(100), telp_one varchar(20), telp_two varchar(20), email varchar(150))`, (err) => {
                        if (err) throw err
                db.run(`CREATE TABLE products(id integer primary key autoincrement,
                     product_name varchar(200) not null unique, product_code varchar(200),
                      barcode varchar(200), category varchar(200),selling_price real,
                       cost_of_product real, product_initial_qty integer, unit varchar(20),
                        check(cost_of_product <= selling_price))`, (err) => {
                        if (err) throw err
                db.run(`CREATE TABLE discount_final1(id integer primary key autoincrement,
                     input_date text, invoice_number varchar(100) not null, discount_percent real,
                      discount_money real, total_discount_final real)`, (err) => {
                        if (err) throw err
                db.run(`CREATE TABLE sales(id integer primary key autoincrement, input_date text,
                     invoice_number varchar(100), buyer varchar(100), buyer_id integer, 
                     payment varchar(6), description text, po_number varchar(100), due_date text,
                      term varchar(100), sales_admin varchar(100), product_name varchar(200) not null, 
                      product_code varchar(200) not null, cost_of_product real, price real, 
                      qty integer, unit varchar(20), discount_percent real, 
                      discount_money real, total real)`, (err) => {
                        if (err) throw err
                db.run(`CREATE TABLE sales_evidence_info(id integer primary key autoincrement,
                     invoice_number varchar(100), print_status varchar(15))`, (err) => {
                        if (err) throw err
                db.run(`CREATE TABLE sales_tax(id integer primary key autoincrement, input_date text,
                     invoice_number varchar(100), total_tax real)`, (err) => {
                        if (err) throw err
                db.run(`CREATE TABLE tax(id integer primary key autoincrement,
                     tax_name varchar(100), percentage real)`, (err) => {
                        if (err) throw err
                db.run(`CREATE TABLE categories(id integer primary key autoincrement, 
                    category varchar(100))`, (err) => {
                        if (err) throw err
                db.run(`CREATE TABLE units(id integer primary key autoincrement, unit varchar(20))`, (err) => {
                        if (err) throw err
                db.run(`insert into profil(store_name, logo) values('Waroengku', 'logoku2.png')`, (err) => {
                        if (err) throw err
                db.run(`insert into users(username,password,access_level,first_name,last_name)
                 values('masgie','masgie','main_user','admin','satu')`,(err) => {
                        if (err) throw err
                db.run(`insert into tax(tax_name, percentage) values('pajak', '10')`, (err) => {
                        if (err) throw err
                        db.run(`insert into units(unit) values('Pack'),('Pcs'),('Kg'),('Lusin'),('Dus'),('Buah'),('Biji'),('Slop'),('Kodi')`, err => {
                            if(err) throw err
                            db.run(`insert into categories(category) values('Electronics'),('Makanan'),('Minuman'),('Gadget'),('Gerabah'),('Pecah-belah'),('Peralatan'),('Snack'),('Lainnya')`, err => {
                                if(err) throw err
                                finilizeTableConfig = () => {
                ...
                if (!login) {
                    db.all(
                        `select * from profil where id = 1 order by id asc`,
                        (err, row) => {
                            if (err) throw err;
                            storeObject.name = row[0].store_name;
                            storeObject.logo = row[0].logo;
    ...
    switch (msgDocId) {
        case "product-data":
            productWindow.webContents.send(
                "update:success",
                "Successfuly updates product data"
            );
            break;
        case "buyer-data":
...
loadToPdf = (
    ...
    db.all(`select * from profil order by id asc limit 1`, (err, row) => {
        if (err) throw err;
        if (row.length < 1) {
            (titleObject.storeName = "Waroengku"),
                (titleObject.storeAddress = "Address"),
                (titleObject.storeLogo = "shop.jpg");
        } else {
            ...
            db.all(
                ...
                (err, row) => {
                    ...
                    if (row.length < 1) {
                        ...
                        db.all(
                            `select * from discount_final1 where invoice_number = '${salesNumber}'`,
                            (err, row) => {
                                if (err) throw err;
                                if (row.length < 1) {
        ...
        (err, rows) => {
            ...
            if (rows.length < 1) {
                ...
                rows.map((row) => {
                    ...
                    salesRecord += `<tr>
                                    <td>${row.product_name} (${row.qty
                        }x${numberFormat(row.price)})</td>
                                    <td>${discountInfo}</td>
...
salesWin = () => {
    ...
    salesWindow = new BrowserWindow({
        ...
        width: width,
        height: height,
        title: "Waroengku | Data Penjualan",
        icon: './assets/images/icons/logoku.ico'
    });
...
salesReportWin = () => {
    ...
    salesReportWindow = new BrowserWindow({
        ...
        width: width,
        height: height,
        title: "Waroengku | Laporan Penjualan",
        icon: './assets/images/icons/logoku.ico'
    });
...
chartWin = () => {
    ...
    chartWindow = new BrowserWindow({
        ...
        width: width,
        height: height,
        title: "Waroengku | Diagram Penjualan",
        icon: './assets/images/icons/logoku.ico'
    });
