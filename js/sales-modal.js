...
submitDiscountFinal = () => {
    ...
    db.all(`select * from discount_final1 where invoice_number = '${sales_number}'`, (err, row) => {
        ...
        if(row.length < 1) {
            ...
            db.run(`insert into discount_final1(input_date, invoice_number, discount_percent, discount_money, total_discount_final) values${values}`, err => {
                if(err) throw err
                ipcRenderer.send('update-success:sales-edit')
            })
        } else {
            values = `('${row[0].id}',datetime('now','localtime'), '${sales_number}', '${discount_percent}', '${discount_money}', '${total_discount_final}')`
            db.run(`replace into discount_final1(id, input_date, invoice_number, discount_percent, discount_money, total_discount_final) values${values}`, err => {
                if(err) throw err
                ipcRenderer.send('update-success:sales-edit')
            })
        }
