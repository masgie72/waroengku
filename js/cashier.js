...
//tampikan nama produk kedalam menu kasir
getProductName = () => {
...
insertSales = () => {
    ...
    if(product_code != "" && product_code != null) {
            ...
            if(alert == 0) {
                ...
                db.all(`select * from sales where product_code = '${prd_code}' and invoice_number = '${sales_number}'`, (err, row) => {
                    ...
                    if(row.length < 1) {
                        ...
                        if(messageBox === 0) {
                            db.run(`insert into sales(input_date, invoice_number, buyer, buyer_id, payment, description, po_number, due_date, term, sales_admin, product_name,
                                product_code, cost_of_product, price, qty, unit, discount_percent, discount_money, total) values(datetime('now','localtime'),'${sales_number}',
                                 '${buyer}','${buyer_id}', '${payment}', '${description}','${po_number}', '${due_data}', '${term}', '','${prd_name}', '${prd_code}',
                                 '${prd_cost}', '${prd_price}','${qty}','${unit}','${discount_percent}','${discount_money}','${total}')`, err => {
                                    if(err) throw err
                        ...
                        let messageBox = dialog.showMessageBoxSync(
                            {
                                ...
                                buttons: [`Yes`, `No`],
                                defaultId: [0, 1],
                                message: `data ${product_code} sudah diinput,Apa anda yakin ingin menambah data?`,
                            }
                        );
                        if(messageBox === 0){
                            db.run(`update sales set input_date = datetime('now', 'localtime'), qty = '${new_qty}', total = '${net_new_total}' where product_code = '${prd_code}'
                                and invoice_number = '${sales_number}'`, err => {
                                    if(err) throw err
                                    loadSales(sales_number)
                                    $('#product_code').val("")
        ...
        dialog.showMessageBoxSync({
            title:'Alert',
            type: 'info',
            message: 'Masukan kode/barcode produk terlebih dahulu'
        }
    )
    }
}
...
totalSales = (sales_number) => {
    ...
    db.all(query, (err, row) => {
        ...
        db.all(`select * from discount_final1 where invoice_number = '${sales_number}'`, (err, row) => {
            if(err) throw err
            if(row.length < 1) {
...
salesTax = (sales_number, tax_rate) => {
    ...
    db.all(`select sum(total) as total_sales from sales where invoice_number = '${sales_number}'`, (err, row) => {
        ...
        if(row.length < 1) {
            ...
            let total_sales = row[0].total_sales
            db.all(`select total_discount_final from discount_final1 where invoice_number = '${sales_number}'`, (err, row) => {
                if(err) throw err
                if(row.length < 1) {
