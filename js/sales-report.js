yearSelect = () => {
    ...
    loadReport = (reportSpan, startMonth, startYear) => {
        ...
        switch(reportSpan) {
            case 'annual':
                query = `select sales.*, sum(discount_final1.total_discount_final) as discount_final from 
                ...
                as sales
                left join
                discount_final1 on sales.date = substr(date(discount_final1.input_date),1,7)`
                break;
            case 'monthly':
                query = `select sales_table.*, sum(discount_final1.total_discount_final) as discount_final from (select date(input_date) as date,
                 sum(total) as total, sum((qty*cost_of_product)) as cogs from sales where substr(date(input_date), 1, 7)
                  = '${startYear}-${startMonth}' group by date) as sales_table left join discount_final1 on sales_table.date =
                   date(discount_final1.input_date) group by date`
                break;
        }
    ...
    exportPdfSalesReport = (filePath, ext, joinIds = false) => {
        ...
        switch(reportSpan) {
            case 'annual':
                ...
                as sales
                left join
                discount_final on sales.date = substr(date(discount_final1.input_date),1,7)`
                break;
            case 'monthly':
                query = `select sales_table.*, sum(discount_final1.total_discount_final) as discount_final from (select date(input_date) as date,
                 sum(total) as total, sum((qty*cost_of_product)) as cogs from sales where substr(date(input_date), 1, 7) = '${startYear}-${startMonth}'
                  group by date) as sales_table left join discount_final1 on sales_table.date = date(discount_final1.input_date) group by date`
                break;
        }
        db.all(query, (err, rows) => {
            if(err) throw err
