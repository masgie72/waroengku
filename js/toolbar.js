...
function deleteAction(id = false, data = false) {
    let msg = `Yakin ingin menghapus ${data} ?`
    ...
    if(id) {
        ...
        if(array_ids.length < 1) {
            let msgBox = dialog.showMessageBoxSync({
              ...
              buttons: ['No', 'Yes'],
              defaultId: [0,1],
              message: 'ATTENTION. You did not select any record. if no record selected, then this action will delete all records in the table. Are you sure want to delete all record in this table?'
