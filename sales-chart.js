...
dailySalesChart = (month, year) => {
    let dailyQuery = `select sales_table.*, sum(discount_final1.total_discount_final) as discount_final from (select date(input_date) as date,
     sum(total) as total, sum((qty*cost_of_product)) as cogs from sales where substr(date(input_date),
      1, 7) = '${year}-${month}' group by date) as sales_table left join discount_final1 on sales_table.date = date(discount_final1.input_date) group by date`
