...
let query
if(searchVal != "") {
    query = `select count(*) as total_row from products where product_name like '%${searchVal}%' escape '!' or product_code like '%${searchVal}%' escape '!' or barcode like '%${searchVal}%' escape '!' or
        category like '%${searchVal}%' escape '!' or selling_price like '%${searchVal}%' escape '!' or cost_of_product like '%${searchVal}%' escape '!'`
    } else {
...
function loadProduct(page_number, total_row_displayed, searchVal) {
    ...
    let query
    if(searchVal != "") {
        query = `select * from products where product_name like '%${searchVal}%' escape '!' or product_code like '%${searchVal}%' escape '!' or barcode like '%${searchVal}%' escape '!' or
        category like '%${searchVal}%' escape '!' or selling_price like '%${searchVal}%' escape '!' or cost_of_product like '%${searchVal}%' escape '!' order by id desc limit ${row_number}
        , ${total_row_displayed}`
    } else {
    ...
    if(required_array.length < 4) {
        ...
        db.serialize( () => {
            db.each(`select count(*) as row_number from products where product_name = '${prd_name}'`, (err, res) => {
                if(err) throw err
                if(res.row_number < 1) {
                    db.run(`insert into products(product_name, barcode, category, selling_price, cost_of_product, product_initial_qty, unit) values('${prd_name}','${prd_barcode}','${prd_category}'
                        ,'${prd_price}','${prd_cost}','${prd_init_qty}','${prd_unit}')`, err => {
                            if(err) throw err
                ...
                if(err) {
                    ...
                    db.all(sql, (err, result) => {
                        if(err) {
                            ...
                            <div class="mb-3">
                            <input type="text" value="${row.product_initial_qty}" placeholder="Stok Awal" id="editPrdInitQty" class="form-control form-control-sm">
                            </div>
                            <div class="d-grid gap-2">
                            <button class="btn btn-sm btn-primary btn-block" onclick="submitEditPrdData(${id})" id="btn-submit-edit"><i class="fa fa-paper-plane"></i>
                            Submit</button>
                            </div>
                            `
